...
def check_tx_hash_coverage(tx_data, to_shard_start, to_shard_end):
    ...
    for local_shard, tx_list in tx_data.items():
        if to_shard_start <= local_shard <= to_shard_end:
            # 获取当前分片的 tx_hash 集合
...
def main():
    ...
    check_tx_hash_coverage(tx_data, to_shard_start, to_shard_end)
